...
def parse_log_file(filepath, progress_callback=None):
    ...
    dev_arr = np.sqrt(
        np.sum((coords_arr - mean_coords) ** 2, axis=1, dtype=np.float64)
    ).astype(np.float32)

    # Vectorized Manhattan jump distance calculation
    if len(coords_arr) > 1:
        diffs = np.abs(np.diff(coords_arr, axis=0))
        # Jump occurs if Manhattan distance > 1 or max step > 1
