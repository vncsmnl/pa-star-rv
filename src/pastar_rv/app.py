"""
PA-Star Runtime Visualizer — Main GUI Application
...
P = {
    "a": "#1f77b4",  # blue — Baseline A
    "b": "#d62728",  # red — Candidate B
    "warn": "#ff7f0e",
}
...
class LogParseWorker(QObject):
    ...
    def run(self):
        ...
        try:
            data = parse_log_file(self.filepath, progress_callback=lambda p: self.progress.emit(p))
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PA-Star Runtime Visualizer — Comparative Analysis & Performance")
    ...
    def _build_ui(self):
        ...
        self._canvases["summary"] = CanvasSummary()
        self._canvases["savings"] = CanvasSavings()
        self._canvases["footprint"] = CanvasFootprint()
        self._canvases["heuristic"] = CanvasHeuristicComparison()
    ...
    def _status_ready(self):
        ...
        if self.data_a:
            parts.append(
                f"A: {self.label_a} ({len(self.data_a['iterations']):,} nodes / {self.data_a['num_jumps']:,} displacements)"
            )
        if self.data_b:
            parts.append(
                f"B: {self.label_b} ({len(self.data_b['iterations']):,} nodes / {self.data_b['num_jumps']:,} displacements)"
            )
    ...
    def _update_active_tab(self):
        ...
        # Dual-file comparative tabs
        if key in ("summary", "savings", "footprint", "heuristic"):
            if self.data_a and self.data_b:
                self._canvases[key].set_data(self.data_a, self.data_b, self.label_a, self.label_b)
        elif key == "band":
            if self.data_a and self.data_b:
                self._canvases["band"].set_data_comparison(
                    self.data_a, self.data_b, self.label_a, self.label_b
                )
            else:
                d, lbl = self._single_data()
                if d is not None:
                    self._canvases["band"].set_data(d, lbl)
        elif key in ("density", "dynamics", "classic"):
            d, lbl = self._single_data()
            if d is not None:
                self._canvases[key].set_data(d, lbl)
    ...
    def _load(self, fp, side):
        ...
        # Invalidate shared comparison cache to release previous derived arrays
    ...
    def _on_parse_done(self, side, data, name, job):
        ...
        # If both loaded, switch to Summary tab (index 0) if currently on single-view tab
        if self.data_a and self.data_b and self.tabs.currentIndex() > 4:
    ...
    def open_a(self):
        fp, _ = QFileDialog.getOpenFileName(
            self, "Open Log — A (Baseline)", "", "Text Files (*.txt);;All Files (*)"
        )
        if fp:
            self._load(fp, "a")
    ...
    def open_b(self):
        fp, _ = QFileDialog.getOpenFileName(
            self, "Open Log — B (Candidate)", "", "Text Files (*.txt);;All Files (*)"
        )
        if fp:
            self._load(fp, "b")
    ...
    def export_all(self):
        ...
        folder = QFileDialog.getExistingDirectory(self, "Select Folder to Export All Tabs")
        if not folder:
            return
        ...
        for key, _ in TAB_DEFS:
            canvas_widget = self._canvases[key]
            if (
                key in ("summary", "savings", "footprint", "heuristic")
                and self.data_a
                and self.data_b
            ):
                canvas_widget.set_data(self.data_a, self.data_b, self.label_a, self.label_b)
            elif key == "band":
                if self.data_a and self.data_b:
                    canvas_widget.set_data_comparison(
                        self.data_a, self.data_b, self.label_a, self.label_b
                    )
                elif d_single is not None:
                    canvas_widget.set_data(d_single, l_single)
            elif key in ("density", "dynamics", "classic") and d_single is not None:
                canvas_widget.set_data(d_single, l_single)
